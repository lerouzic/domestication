...
for (mysim in scenarios) {
  ...
  plot.norm(mysim,ylim=ylim.norm,xaxt="n", yaxt=if(firstcol) "s" else "n", xlab="", ylab=if(firstcol) ylab.norm else "", xpd=if(firstcol) NA else FALSE)
  ...
  plot.inout.gainloss(mysim, deltaG=deltaG, ylim=ylim.inout.gainloss,xaxt="n", yaxt=if(firstcol) "s" else "n", xlab="", ylab=if(firstcol) ylab.inout.gainloss else "", xpd=if(firstcol) NA else FALSE)
  bottleneck.plot(Ndyn.all[["default"]], y=19, lwd=2)
  selectionchange.plot(meansim.all[["default"]], y=19, cex=1.5)
  
  plot.nconn(mysim, ylim=ylim.nconn, xaxt="n", yaxt=if(firstcol) "s" else "n", xlab="", ylab=if(firstcol) ylab.nconn else "", xpd=if(firstcol) NA else FALSE)
  bottleneck.plot(Ndyn.all[["default"]], y=0, lwd=2)
  selectionchange.plot(meansim.all[["default"]], y=0, cex=1.5)
  
  plot.Gdiff(mysim, deltaG=deltaG, ylim=ylim.Gdiff,xaxt="n", yaxt=if(firstcol) "s" else "n", xlab="", ylab=if(firstcol) ylab.Gdiff else "", xpd=if(firstcol) NA else FALSE)
  bottleneck.plot(Ndyn.all[["default"]], y=0, lwd=2)
  selectionchange.plot(meansim.all[["default"]], y=0, cex=1.5)
  generation.axis()	
}